...
def find_all_home_weather(url,access_key, cur, conn):
    ...
    for date, temperature in dic.items():
        cur.execute("INSERT OR IGNORE INTO game_weather (date, temperature) VALUES (?, ?)", (date, temperature))
        conn.commit()
...
def main():
    ...
    cur, conn = setUpDatabase('weather_project.db')
    create_weather_table(cur, conn)
    find_all_home_weather(access_key, url, cur, conn)
    ...
    cur.execute("CREATE TABLE IF NOT EXISTS game_weather_new AS SELECT * FROM game_weather ORDER BY date ASC;")
    cur.execute("DROP TABLE IF EXISTS game_weather;")
